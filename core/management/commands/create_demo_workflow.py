...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"  - admin / admin123 (Администратор)")
        self.stdout.write(f"  - storekeeper / storekeeper123 (Кладовщик)")
        self.stdout.write(f"  - picker / picker123 (Сборщик мелких деталей)")
        self.stdout.write(f"  - loader / loader123 (Грузчик)")
        self.stdout.write(f"  - sales / sales123 (Менеджер по продажам)")
        self.stdout.write(f"  - analyst / analyst123 (Аналитик)")
        self.stdout.write("\nСозданные данные:")
